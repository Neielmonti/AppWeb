...
if not BASE_PATH:
    raise ValueError("❌ BASE_PATH no se cargó desde el archivo .env")
...
def entrenar_modelo():
    ...
    plt.xlabel('Épocas')
    plt.ylabel('Valor')
    plt.title('Evolución del aprendizaje del modelo')
    plt.legend()
    plt.savefig(os.path.join(SPLIT_PATH, f"learning_curve_{timestamp}.png"))
    plt.close()

    test_loss, test_acc = model.evaluate(test_ds)
    print(f"\n📊 Exactitud global: {test_acc:.3f}")

    model_name = f"modelo_soybean_imagenet_{timestamp}.keras"
    ...
    plt.ylabel("Real")
    plt.savefig(os.path.join(SPLIT_PATH, f"confusion_matrix_{timestamp}.png"))
    plt.close()

    # Guardar matriz como CSV
    cm_df = pd.DataFrame(cm, index=class_names, columns=class_names)
    cm_df.to_csv(os.path.join(SPLIT_PATH, f"confusion_matrix_{timestamp}.csv"))
