...
# Full HD vertical por defecto
TAM_OBJ = (1920, 1080)  # (ancho, alto)
...
def estandarizar_dataset(carpeta_entrada, carpeta_salida, tamaño=TAM_OBJ, paso=10):
    """
    Recorre toda la carpeta con subdirectorios y estandariza todas las imágenes.
    Muestra progreso cada 'paso' imágenes.
    """
    procesadas = []
    contador = 0
    total = sum(len([f for f in files if os.path.splitext(f)[1].lower() in ALLOWED_EXT])
    ...
    for root, _, files in os.walk(carpeta_entrada):
        for file in files:
            ...
            if ext in ALLOWED_EXT:
                ...
                if estandarizar_imagen(ruta_in, ruta_out, tamaño=tamaño):
